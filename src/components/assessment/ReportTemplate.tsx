...
export const ReportTemplate = React.forwardRef<HTMLDivElement, ReportTemplateProps>(
  ({ assessmentResult, studentName }, ref) => {
    ...
    const ScoreComparisonBarChart = () => (
      <div className="relative w-full h-48">
        <svg width="280" height="190" viewBox="0 0 280 190" className="overflow-visible">
          ...
          <g>
            {/* Face avant */}
            <rect x="140" y={150 - scoreData.average * 25} width="30" height={scoreData.average * 25} fill="#DC143C" stroke="black" strokeWidth="0.5" />
            {/* Face droite */}
    ...
    const TimeDistributionPieChart = () => {
        const radius = 50; // Réduit légèrement le rayon pour mieux s'adapter
        const center = { x: 50, y: 70 }; // Ajusté la position y du centre
    ...
    return (
      <div ref={ref} className="w-[794px] h-auto bg-white text-black font-sans leading-tight text-sm">
        {/* Header avec couleurs */}
        <div className="flex justify-between items-start mb-4 px-6 pt-6">
          <div className="flex-1">
            <div className="flex items-center mb-2">
              <h1 className="text-4xl font-black tracking-wider mr-4">
                <span style={{ color: '#E91E63' }}>COMPLE</span>
                <span style={{ color: '#4CAF50' }}>METRICS</span>
                <span className="text-lg align-top">®</span>
              </h1>
              <svg width="40" height="40" viewBox="0 0 40 40" className="ml-2">
                <circle cx="20" cy="8" r="6" fill="#4169E1" />
                <text x="20" y="12" textAnchor="middle" fill="white" fontSize="8" fontWeight="bold">Q</text>
                <circle cx="8" cy="20" r="6" fill="#4169E1" />
                <text x="8" y="24" textAnchor="middle" fill="white" fontSize="8" fontWeight="bold">a</text>
                <circle cx="32" cy="20" r="6" fill="#4169E1" />
                <text x="32" y="24" textAnchor="middle" fill="white" fontSize="8" fontWeight="bold">b</text>
                <circle cx="20" cy="32" r="6" fill="#4169E1" />
                <text x="20" y="36" textAnchor="middle" fill="white" fontSize="8" fontWeight="bold">mc</text>
                <polygon points="20,15 25,25 15,25" fill="#4CAF50" />
              </svg>
              <div className="ml-2">
                <div className="text-blue-600 font-bold text-lg">Radiant Prep</div>
              </div>
            </div>
            <div style={{
            background: '#dc2626',
            ...
            <div className="mt-2">
              <div className="flex items-center">
                <span className="text-lg font-bold">K-12</span>
                <div className="flex items-center mx-2">
                  {['S', 'C', 'O', 'R', 'E'].map((letter, i) => (
                    <span key={i} className="w-6 h-6 border-2 border-black rounded-full flex items-center justify-center text-xs font-bold mx-0.5">
                      {letter}
                    </span>
                  ))}
                </div>
        ...
        <div className="px-6 mb-6">
          <div className="flex">
            <div className="w-2/3 bg-black text-white p-4">
              <div className="bg-yellow-400 text-black px-2 py-1 text-sm font-bold mb-3 inline-block">
                GENERAL ANALYSIS / FOCUS AREAS:
              </div>
              <p className="text-sm leading-relaxed">
                Data suggests that Inaaya requires significant intervention in mathematics. 
                As she is currently lagging behind peers, focus areas include: solidifying 
                foundation skills and operational fluency with decimals, fractions and mixed 
                numbers, as well as work with real-world problems.
              </p>
            </div>
            ...
            <div className="w-1/3 bg-black text-white p-4">
              ...
              <p className="text-sm">Varies based on service options</p>
            </div>
          </div>
        </div>
