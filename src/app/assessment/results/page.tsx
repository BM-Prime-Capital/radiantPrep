...
import {
  CheckCircle2, XCircle, Trophy, BookOpen, Calculator,
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    if (!authLoading && isAuthenticated && !assessmentResult) router.replace('/assessment/select');
  }, [isAuthenticated, authLoading, assessmentResult, router]);

  // Loading state wrapped in ChildLayout so header/sidebar restent visibles
  ...
  // Gère 'GRADE_1' ou 1
  const gradeLabel = typeof grade === 'string' ? (grade.match(/\d+/)?.[0] ?? grade) : grade;
  ...
  const performanceData = {
    ...
    'needs-improvement': {
      ...
      color: 'bg-gradient-to-r from-primary to-accent rounded-xl p-6 text-white',
    ...
    <ChildLayout>
      <div className="max-w-6xl mx-auto px-4 py-8 sm:py-12">
        ...
        <motion.div
          ...
          className={cn('rounded-2xl p-8 mb-10 text-white shadow-xl', performanceData[performanceLevel].color)}
        >
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-3 mb-4">
                {performanceData[performanceLevel].icon}
                <h1 className="text-3xl sm:text-4xl font-bold">{performanceData[performanceLevel].message}</h1>
              </div>
        ...
        <div className="flex flex-wrap gap-4 mb-10">
          ...
          <ReportGenerator assessmentResult={assessmentResult} studentName={studentName} />
        </div>
            ...
            <Card className="h-full shadow-sm border border-gray-200">
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  <Progress value={percentage} className="h-2 bg-gray-200 [&>div]:bg-gradient-to-r [&>div]:from-blue-500 [&>div]:to-indigo-600" />
                </div>
              </CardContent>
            </Card>
          </motion.div>
                                ...
                                <div>
                                  ...
                                  <p
                                    className={cn(
                                      ...
                                      answer.isCorrect ? 'border-green-200 text-green-700' : 'border-red-200 text-red-700'
                                    )}
                                  >
